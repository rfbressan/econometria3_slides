...
a) Mostre que a especificação não-linear do logit para a probabilidade de resposta positiva implica em um modelo **linear** para _log-odds ratio_ (razão entre probabilidades de sucesso e fracasso em logarítimo), $\log\left(\frac{p(\symbf{x})}{1-p(\symbf{x})}\right)$.
...
a) Estime a probabilidade de um estudante que estuda 40 horas e teve 7,0 como nota de econometria 1 de receber um A.
...
b) Quantas horas o aluno da parte a) precisaria estudar para ter 50% de chance de tirar A na disciplina?
...
você observou uma amostra $\{x_i\}_{i=1}^N$.

a) Escreva a função de log-verossimilhança para estimar o parâmetro $\theta$.

b) Derive o estimador MLE de $\theta$, $\hat\theta$.

c) Derive o estimador de variância de $\hat\theta$.
...
a) O saldo do cartão parece ser relevante para determinar a situação de inadimplência? Qual a direção da previsão?

b) no modelo MPL, a partir de qual valor de saldo a previsão de probabilidade passa a ser negativa?

c) Sabendo que a média dos saldos e o percentil 75% são respectivamente de, `r mean_x` e `r q75_x`, calcule o efeito do aumento de R\$ 1.000 na probabilidade de inadimplência, tanto para MPL quanto para Logit, nestes pontos da distribuição. DICA: a distribuição logística é $\Lambda(z)=\frac{\exp(z)}{1+\exp(z)}$

d) Com base na matriz de confusão apresentada abaixo calcule o percentual cometido de erros do tipo I e II e responda, qual modelo você prefere utilizar e por que?
