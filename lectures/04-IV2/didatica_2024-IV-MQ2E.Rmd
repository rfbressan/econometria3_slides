...
* Podemos ter mais de um instrumento para $s_i$.

--

* Neste caso, temos o que chamamos de **sobre-identificação**.

--

* A estimação **deve** ser realizada por MQ2E, com o primeiro estágio da seguinte forma:

$$s_i = \alpha_0 + \alpha_1 z_{1i} + \alpha_2 z_{2i} + \alpha_3 x_i + \eta_i$$
...
* Interessados em saber se os instrumentos são *conjuntamente* significativos.

$$s_i=\alpha_0 + \alpha_1 z_{1i} + \alpha_2 z_{2i} + \alpha_3 x_i + \eta_i$$

--

* A F-Stat compara o poder preditivo do primeiro estágio com $(\alpha_1, \alpha_2\neq 0)$ e sem as VIs $(\alpha_1,\alpha_2=0)$. 

* Se os modelos tiverem poder preditivo semelhante, a F-stat será baixa e não rejeitamos H0. VIs são **conjuntamente insignificantes** no modelo de primeiro estágio. 
...
* VI é viesado, porém **consistente sob determinadas premissas**.

--

* Mesmo com $Corr(z,u)$ muito pequena, podemos errar.

* Pequena correlação entre $x$ e $z$ pode produzir estimativas **inconsistentes**.

$$
\text{plim}\left(\hat{\beta}_1^{VI}\right) = \beta_1 + \frac{Corr(z,u)}{Corr(z,x)} \cdot \frac{\sigma_u}{\sigma_x}
$$

--


* Um **instrumento fraco** é aquele com pequeno valor absoluto para $Corr(z,x)$.
...
# Leitura Recomendada `r emo::ji("books")`
