...
1. Dê uma olhada no conjunto de dados e liste as variáveis. Verifique as definições de variáveis com `?njmin`.

1. Tabule o número de lojas por `state` e por onda de pesquisa (`observation`). Corresponde ao que está na *Tabela 1* do [paper](http://davidcard.berkeley.edu/papers/njmin-aer.pdf)?
...
1. Crie uma variável de funcionários equivalente em tempo integral (FTE) chamada `empfte` igual a `empft` + 0,5*`emppt` + `nmgrs`. `empft` e `emppt` correspondem, respectivamente, a funcionários em tempo integral e meio período. `nmgrs` é o número de gerentes.
...
1. Calcule o número médio de empregos FTE, porcentagem média de funcionários FT (fora do número de funcionários FTE) e salário inicial médio (`wage_st`) por estado e por onda de pesquisa. Compare seus resultados com a *Tabela 2* do artigo.

```{r , echo=FALSE, eval=FALSE, results='hide'}
ck1994_df |> 
    group_by(state, observation) |> 
    summarise(mean_fte = mean(empfte, na.rm = TRUE),
              pct_ft = mean(empft/empfte, na.rm = TRUE),
              mean_st = mean(wage_st, na.rm = TRUE))
```

5. Quão diferentes são os restaurantes de fast-food de Nova Jersey e da Pensilvânia antes do aumento do salário mínimo?
...
1. Crie uma variável dummy, `treat`, igual a `FALSE` se `state` é Pensilvânia e `TRUE` se Nova Jersey.

```{r , echo=FALSE, results='hide'}
ck1994_df <- ck1994_df |> 
    mutate(treat = state == "New Jersey")
```

1. Crie uma variável dummy, `post`, igual a `FALSE` se `observation` é fevereiro de 1992 e `TRUE` caso contrário.

```{r , echo=FALSE, results='hide'}
ck1994_df <- ck1994_df |> 
    mutate(post = observation != "February 1992")
```

1. Estime o seguinte modelo de regressão. Você obtém os mesmos resultados do slide 9?
...
reg <- lm(empfte~treat+post+treat:post, data = ck1994_df)
summary(reg)
```
...
Com a Lista 4 feita! `r emo::ji("white_check_mark")`
...
| <a href="https://raw.githack.com/rfbressan/econometria3_slides/master/lectures/07-DID/lista_IV_pt.html">.ScPored[<i class="fa fa-list fa-fw"></i>] | Lista de Exercícios I |
