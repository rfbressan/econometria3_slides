...
date: "UDESC/Esag </br> Data devida: 08/12/2022"
...
a) Definimos as seguintes variáveis: $y= Y-\bar{Y}$, $s= State-\bar{State}$, $d= D-\bar{D}$. Além destas, definimos $n_s$ como o número de observações onde $State = 1$ e $n_d$ quando $D=1$. Veja que $D=1$ é um subconjunto de $State=1$, além do mais, supondo painel balanceado, o número de observações onde $D=1$ **quando** $State = 1$ é o mesmo que $D=0$ quando $State=1$.

$$
\begin{align}
\hat{\theta}&=\frac{\sum yd\sum s^2-\sum ys\sum sd}{\sum s^2 \sum d^2 - (\sum sd)^2}
\end{align}
$$
Calculando todos os termos

$\sum yd = \sum_{D=1}y-\bar{D}\sum y$

$\sum s^2 = \sum S - NT\bar{S}^2=n_s(1-\bar{S})$

$\sum ys=\sum_{S=1}y-\bar{S}\sum y$

$\sum sd=\sum SD-\bar{S}\sum D-\bar{D}\sum S+NT\bar{S}\bar{D}=n_d(1-\bar{S})$

Desta forma, considerando que $n_s=2 n_d$, o numerador de $\hat{\theta}$ se reduz a:

$$n_d(1-\bar{S})\left(\sum_{S=1,D=1}y - \sum_{S=1,D=0}y\right).$$
O denominador será $n_d^2(1-\bar{S})$, e portanto, o estimador de MQO é:

$$
\begin{equation}
\hat{\theta}=\frac{\sum_{S=1,D=1}y}{n_d}-\frac{\sum_{S=1,D=0}y}{n_d}.
\end{equation}
$$
Como o painel é balanceado, $n_d$ representa tanto o número de estados no grupo tratamento efetivamente tratados (pós) quanto não ainda tratados (pré). Imediatamente percebemos que este é um estimador de uma diferença apenas, dentro do grupo tratado somente.

b) Em termos populacionais passamos o operador de valor esperado condicional sobre nossa equação de regressão para extrair o valor de $\theta$ dado $State$ e $D$.

$E[y_{it}|State_i=1, D_{it}=0]=\beta_0+\beta_1$

$E[y_{it}|State_i=1, D_{it}=1]=\beta_0+\beta_1+\theta$, logo:

$\theta=E[y_{it}|State_i=1, D_{it}=1] - E[y_{it}|State_i=1, D_{it}=0]$. Percebam como apenas a amostra tratada $S=1$ entra no cômputo de $\theta$. O estimador de MQO derivado no item anterior é facilmente identificado como o análogo amostral do $\theta$ populacional.

c) Não é um estimador DID. Ele toma apenas uma diferença, no grupo tratado.

d) Para $\hat\theta$ ser equivalente a um estimador DID, devemos ter que a diferença temporal no grupo controle seja igual a zero, isto é

$E[y_{it}|State_i=0, Post_t=1] - E[y_{it}|State_i=0, Post_t=0]=0.$

Em outras palavras, o contrafactual deve ser constante, não apresentando tendência temporal.
